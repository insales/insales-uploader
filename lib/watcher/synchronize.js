...
function syncFile(fileInfo, syncPath, instance, file) {
  ...
  let encoding = (prop == 'content') ? 'utf8' : 'base64';
  ...
  if (isString) {
    ...
    identSize = fileSizeInBytes2 === fileSizeInBytes2;
  }

  // true если синхронизован
  let isSync = (isString && (!identSize)) ? jsdiff.diffChars(fileProps[prop], syncFileProps[prop]).length === 0 : false;
