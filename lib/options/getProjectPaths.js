...
export {styleExt, scriptsExt, svgExt, imgExt, fontsExt};

/**
 * Получить все возможные директории для проекта
 */
export function getPaths (options) {
  ...
  const pathsDefault = {
    ...
    folders: {},
    files: `${root}/files/`,
    widget_types: `${root}/widget_types/`,
    foldersDefaults: {
  ...
  Object.values(pathsDefault.folders).forEach((folder) => {
    let _folder = (folder.includes('widget_types')) ? `${folder}**/` : folder;
    let seeFolder = `${_folder}*.*`;
  ...
  pathsDefault.assets = {
    'Asset::Widget': {
      'extension': ['.json', '.jpg', '.liquid'],
      'backup': upath.normalize(`${root}/backup/widget_types/`),
      'folder': pathsDefault.widget_types
    },
    'Asset::Template': {
      'extension': '.liquid',
